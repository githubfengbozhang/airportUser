    ...
    <el-form ref="loginForm" :model="loginForm" :rules="loginRules" class="login-form" auto-complete="on" label-position="left">
      ...
      <div class="lang">
        <el-radio-group v-model="language" size="mini">
          <el-radio v-for="item of lang" :key="item.key" :label="item.value" border>{{ item.label }}</el-radio>
        </el-radio-group>
      </div>
    </el-form>
  </div>
...
export default {
  ...
  data() {
    ...
    return {
      language: 0,
      lang: [{
        label: this.$t('login.zh'), // 模板语法的一种
        value: 0
      }, {
        label: this.$t('login.en'),
        value: 1
      }],
      loginForm: {
        username: 'admin',
  ...
  watch: {
    ...
    language: function(val) { // 侦听单选按钮的变化，从而进行切换语言
      ...
      Vue.set(this.lang, 0, { label: this.$t('login.zh'), value: 0 })
      Vue.set(this.lang, 1, { label: this.$t('login.en'), value: 1 })
      /**
        this.lang: [{
